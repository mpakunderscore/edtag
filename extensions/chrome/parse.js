function parse(page) {
	...
	while (match = regex.exec(text)) {
		...
		if (word.substr(word.length - 1, word.length) === 's' 
			&& words_map[word.substr(0, word.length - 1)] != null
		)
				words_map[word.substr(0, word.length - 1)]++;
		
		else if (words_map[word] == null) words_map[word] = 1;
		else words_map[word]++;
	}
